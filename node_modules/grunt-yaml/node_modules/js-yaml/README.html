<h1>JS-YAML - YAML 1.2 parser and serializer for JavaScript</h1><p><a href="http://travis-ci.org/nodeca/js-yaml"><img src="https://secure.travis-ci.org/nodeca/js-yaml.png" alt="Build Status"></a></p>
...
<h2>Installation</h2><h3>YAML module for node.js</h3><pre><code>npm install js-yaml
</code></pre><h3>CLI executable</h3><p>If you want to inspect your YAML files from CLI, install js-yaml globally:</p>
<pre><code>npm install -g js-yaml
</code></pre><h4>Usage</h4><pre><code>usage: js-yaml [-h] [-v] [-c] [-t] file
...
</code></pre><h3>Bundled YAML library for browsers</h3><pre><code class="language-html">&lt;!-- esprima required only for !!js/function --&gt;
...
<p>Notes:</p>
<ol>
<li>We have no resourses to support browserified version. Don&#39;t expect it to be
...
<h2>API</h2><p>Here we cover the most &#39;useful&#39; methods. If you need advanced details (creating
...
<h3>safeLoad (string [ , options ])</h3><p><strong>Recommended loading way.</strong> Parses <code>string</code> as single YAML document. Returns a JavaScript
...
<li><code>strict</code> <em>(default - false)</em> makes the loader to throw errors instead of
warnings.</li>
<li><code>schema</code> _(default: <code>DEFAULT_SAFE_SCHEMA</code>)_ - specifies a schema to use.<ul>
...
It allows numbers in any notaion, use <code>Null</code> and <code>NULL</code> as <code>null</code>, etc.
Core schema also has no such restrictions. It allows binary notation for integers.</p>
<h3>load (string [ , options ])</h3><p><strong>Use with care with untrusted sources</strong>. The same as <code>safeLoad()</code> but uses
...
<h3>safeLoadAll (string, iterator [ , options ])</h3><p>Same as <code>safeLoad()</code>, but understands multi-document sources and apply
...
<h3>loadAll (string, iterator [ , options ])</h3><p>Same as <code>safeLoadAll()</code> but uses <code>DEFAULT_FULL_SCHEMA</code> by default.</p>
<h3>safeDump (object [ , options ])</h3><p>Serializes <code>object</code> as YAML document. Uses <code>DEFAULT_SAFE_SCHEMA</code>, so it will
...
<li><code>schema</code> _(default: <code>DEFAULT_SAFE_SCHEMA</code>)_ specifies a schema to use.</li>
</ul>
<p>styles:</p>
...
<h3>dump (object [ , options ])</h3><p>Same as <code>safeDump()</code> but without limits (uses <code>DEFAULT_FULL_SCHEMA</code> by default).</p>
<h2>Supported YAML types</h2><p>The list of standard YAML tags and corresponding JavaScipt types. See also
...
</code></pre><h2>Caveats</h2><p>Note, that you use arrays or objects as key in JS-YAML. JS do not allows objects
...
<h2>Breaking changes in 2.x.x -&gt; 3.0.x</h2><p>If your have not used <strong>custom</strong> tags or loader classes and not loaded yaml
files via <code>require()</code> - no changes needed. Just upgrade library.</p>
<p>In other case, you should:</p>
<ol>
...
<h2>License</h2><p>View the <a href="https://github.com/nodeca/js-yaml/blob/master/LICENSE">LICENSE</a> file
(MIT).</p>
