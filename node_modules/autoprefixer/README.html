<h1>Autoprefixer <a href="https://travis-ci.org/postcss/autoprefixer"><img src="https://travis-ci.org/postcss/autoprefixer.svg" alt="Build Status"></a></h1><p><img align="right" width="94" height="71"
...
from <a href="http://caniuse.com/">Can I Use</a>. It is <a href="https://developers.google.com/web/tools/setup/setup-buildtools#dont_trip_up_with_vendor_prefixes">recommended</a> by Google and used in Twitter and Taobao.</p>
...
<h2>Features</h2><h3>Write Pure CSS</h3><p>Working with Autoprefixer is simple: just forget about vendor prefixes
...
<h3>Flexbox, Filters, etc.</h3><p>Just write normal CSS according to the latest W3C specs and Autoprefixer
...
<h3>Only Actual Prefixes</h3><p>Autoprefixer utilizes the most recent data from <a href="http://caniuse.com/">Can I Use</a>
...
<h2>Browsers</h2><p>Autoprefixer uses <a href="https://github.com/ai/browserslist">Browserslist</a>, so you can specify the browsers
you want to target in your project by queries like <code>last 2 versions</code>
or <code>&gt; 5%</code>.</p>
<p>The best way to provide browsers is <code>browserslist</code> config
...
and default value.</p>
<h2>Outdated Prefixes</h2><p>By default, Autoprefixer also removes outdated prefixes.</p>
...
<pre><code class="language-js">var cleaner  = postcss([ autoprefixer({ add: false, browsers: [] }) ]);
...
<h2>FAQ</h2><h4>No prefixes in production</h4><p>Many other tools contain Autoprefixer. For example, webpack uses Autoprefixer
...
<p>You need to put your browsers to <a href="https://github.com/ai/browserslist#config-file"><code>browserslist</code> config</a> in project root —
as result all tools (Autoprefixer, cssnano, doiuse, cssnext) will use same
browsers list.</p>
<h4>What is unprefixed version for <code>-webkit-min-device-pixel-ratio</code>?</h4><pre><code class="language-css">@media (min-resolution: 2dppx) {
    .image {
...
<pre><code class="language-css">@media (-webkit-min-device-pixel-ratio: 2),
...
<h4>Does it add polyfills?</h4><p>No. Autoprefixer only adds prefixes.</p>
...
<h4>Why doesn’t Autoprefixer add prefixes to <code>border-radius</code>?</h4><p>Developers are often surprised by how few prefixes are required today.
...
<h4>Why Autoprefixer uses unprefixed properties in <code>@-webkit-keyframes</code>?</h4><p>Browser teams can remove some prefixes before others. So we try to use
all combinations of prefixed/unprefixed values.</p>
<h4>How to work with legacy <code>-webkit-</code> only code?</h4><p>Autoprefixer needs unprefixed property to add prefixes. So if you only
...
<h4>Does Autoprefixer add <code>-epub-</code> prefix?</h4><p>No, Autoprefixer works only with browsers prefixes from Can I Use.
But you can use <a href="https://github.com/Rycochet/postcss-epub">postcss-epub</a>
for prefixing ePub3 properties.</p>
<h4>Why doesn’t Autoprefixer transform generic font-family <code>system-ui</code>?</h4><p><code>system-ui</code> is technically not a prefix and the transformation is not future-proof. But you can use <a href="https://github.com/JLHwung/postcss-font-family-system-ui">postcss-font-family-system-ui</a> to transform <code>system-ui</code> to a practical font-family list.</p>
<h2>Usage</h2><h3>Gulp</h3><p>In Gulp you can use <a href="https://github.com/postcss/gulp-postcss">gulp-postcss</a> with <code>autoprefixer</code> npm package.</p>
<pre><code class="language-js">gulp.task(&#39;autoprefixer&#39;, function () {
...
<h3>Webpack</h3><p>In <a href="http://webpack.github.io/">webpack</a> you can use <a href="https://github.com/postcss/postcss-loader">postcss-loader</a> with <code>autoprefixer</code>
...
<h3>Grunt</h3><p>In Grunt you can use <a href="https://github.com/nDmitry/grunt-postcss">grunt-postcss</a> with <code>autoprefixer</code> npm package.</p>
...
<h3>CSS-in-JS</h3><p>There is <a href="https://github.com/postcss/postcss-js">postcss-js</a> to use Autoprefixer in React Inline Styles, <a href="https://github.com/blakeembrey/free-style">Free Style</a>,
Radium and other CSS-in-JS solutions.</p>
<pre><code class="language-js">let prefixer = postcssJs.sync([ autoprefixer ]);
let style = prefixer({
    display: &#39;flex&#39;
});
</code></pre>
...
<h3>CLI</h3><p>You can use the <a href="https://github.com/postcss/postcss-cli">postcss-cli</a> to run Autoprefixer from CLI:</p>
<pre><code class="language-sh">npm install --global postcss-cli autoprefixer
postcss *.css --use autoprefixer -d build/
</code></pre>
<p>See <code>postcss -h</code> for help.</p>
<h3>JavaScript</h3><p>You can use Autoprefixer with <a href="https://github.com/postcss/postcss">PostCSS</a> in your Node.js application
or if you want to develop an Autoprefixer plugin for new environment.</p>
<pre><code class="language-js">var autoprefixer = require(&#39;autoprefixer&#39;);
...
<h3>Text Editors and IDE</h3><p>Autoprefixer should be used in assets build tools. Text editor plugins are not
...
<h2>Warnings</h2><p>Autoprefixer uses the <a href="https://github.com/postcss/postcss/blob/master/docs/api.md#warning-class">PostCSS warning API</a> to warn about really important problems
in your CSS:</p>
<ul>
...
<pre><code class="language-js">result.warnings().forEach(function (warn) {
    console.warn(warn.toString());
});
</code></pre>
<p>Every Autoprefixer runner should display this warnings.</p>
<h2>Disabling</h2><p>Autoprefixer was designed to have no interface – it just works.
...
<p>Autoprefixer has 4 features, which can be enabled or disabled by options:</p>
<ul>
...
<li><code>grid: true</code> will enable Grid Layout prefixes for IE.</li>
</ul>
...
<pre><code class="language-css">/* autoprefixer: off */
...
<h2>Options</h2><p>Function <code>autoprefixer(options)</code> returns new PostCSS plugin.
See <a href="http://api.postcss.org">PostCSS API</a> for plugin usage documentation.</p>
<pre><code class="language-js">var plugin = autoprefixer({ cascade: false });
</code></pre>
...
<h2>Debug</h2><p>You can check which browsers are selected and which properties will be prefixed:</p>
<pre><code class="language-js">var info = autoprefixer().info();
console.log(info);
</code></pre>
